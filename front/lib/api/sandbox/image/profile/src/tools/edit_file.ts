...
import { ToolError, error, errorWithUsage } from "../shared/errors";
import {
  countOutputLines,
  safeOutput,
} from "../shared/output";
...
function makeDiff(filePath: string, original: string, modified: string): string {
...
function truncateDiffOutput(diffOutput: string): string {
  ...
  const suffix =
    `[Diff truncated after ${countOutputLines(text)} lines. Byte budget: ${MAX_DIFF_BYTES}, line budget: ${MAX_DIFF_LINES}]`;
...
function editOneFile(
  ...
  if (!fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) {
    return { diffOutput: "", exitCode: 1, message: `Error: file not found: ${filePath}` };
  }

  const stat = fs.statSync(filePath);
  if (stat.size === 0) {
    return { diffOutput: "", exitCode: 1, message: `Error: file is empty: ${filePath}` };
  }
  ...
  const effectiveNewText = isMarkdown ? newText : stripTrailingWhitespace(newText);
...
export async function run(
  ...
  if (profile === "openai") {
    error("edit_file is not available for the openai profile; use apply_patch instead");
  }
  ...
  for (const filePath of paths) {
    ...
    if (result.diffOutput) {
      writeToStream(dryRun ? process.stdout : process.stderr, result.diffOutput);
    }
  }
