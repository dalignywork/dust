...
const GREP_USAGE_ANTHROPIC =
  `${GREP_USAGE_BASE} [--output-mode content|files|count] [--case-insensitive] [--max-line-length N]`;
...
function parseGrepArgs(args: readonly string[], profile: Profile): ParsedGrepArgs {
  ...
  return {
    ...
    contextLines: parseIntArg(named.context ?? "0", "--context", { minimum: 0 }),
    fileGlob: named.glob,
    maxLineLength:
      profile === "anthropic"
